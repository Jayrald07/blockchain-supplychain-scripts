...
echo '.channel_group.groups.Application.groups.'$(echo $ORG_NAME)'MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "'$SERVER_IP'","port": "'$PEER_PORT'"}]},"version": "0"}}'

jq '.channel_group.groups.Application.groups.'$(echo $ORG_NAME)'MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "'$SERVER_IP'","port": "'$PEER_PORT'"}]},"version": "0"}}' MSPconfig.json > MSPconfigmodified_config.json
